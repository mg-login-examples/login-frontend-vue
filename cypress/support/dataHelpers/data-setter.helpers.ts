...
export default class DataSetterHelpers {
  ...
  static async createAQuoteIfNoQuoteExists() {
    if ((await AdminAPI.getQuotes()).length === 0) {
      const user = await this.createUserIfNoUserExists('quote_creator@fakemail.com', '12345678')
  ...
  static async create2UserNotesIfAtLeast2NotesDoNotExists(email: string, password: string) {
    const user = await this.createUserIfNoUserExists(email, password)
    if ((await this.getUserNotes(email, password)).length < 2) {
      ...
      await AdminAPI.createUserNote('E2E generated note 2', 'some stuff about some things', user.id)
    }
  }
