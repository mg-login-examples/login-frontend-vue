...
Given(
  ...
  (quoteTextStoreKey: string, username: string) => {
    AllQuotesPage.setInStoreTextOfQuoteNotCreatedOrLikedByUser(quoteTextStoreKey, username)
...
When('I click on like button for the quote with text {string}', (quoteText: string) => {
  quoteText = getValueFromStore(quoteText)
  AllQuotesPage.clickOnLikeButtonOfQuoteTileWithText(quoteText)
})

// Uses webpage extracted value
When('I click on unlike button for the quote with text {string}', (quoteText: string) => {
  quoteText = getValueFromStore(quoteText)
...
Then(
  ...
  (quoteText: string, authorUsername: string) => {
    AllQuotesPage.assertQuoteWithTextAndAuthorIsVisible(quoteText, authorUsername)
  }
)

// Uses webpage extracted value
Then('the like button for the quote with text {string} is visible', (quoteText: string) => {
  quoteText = getValueFromStore(quoteText)
  AllQuotesPage.assertLikeQuoteButtonIsVisibleOnQuoteTileWithText(quoteText)
})
