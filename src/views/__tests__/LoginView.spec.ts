...
const selectors = {
  ...
  showPasswordIcon: "[data-test='login--show-password-button'] font-awesome-icon-stub",
...
describe('views > LoginView.vue', () => {
  ...
  it('handles successful user logins', async () => {
    ...
    expect((wrapper.find(selectors.rememberMeCheckbox).element as HTMLInputElement).checked).toBe(
      false
    )
  ...
  it('renders signup link and forgot password with url', () => {
    ...
    expect(wrapper.find(selectors.signupLink).findComponent(RouterLinkStub).props().to).toBe(
      '/signup'
    )
    expect(
      wrapper.find(selectors.forgotPasswordLink).findComponent(RouterLinkStub).props().to
