...
describe('views > GroupChat.vue', () => {
  ...
  it('subscribes to room chat entry / exit events on component init', () => {
    ...
    expect(webSocketStore.socketManager?.socketEventEmitter.on).toBeCalledTimes(1)
  })
  ...
  it('requests chat room entry based on user entered room input', async () => {
    ...
    expect((wrapper.find(selectors.roomInput).element as HTMLInputElement).value).toBe(chatRoomName)
  ...
  it('enter chat room when channel subscribed successfully message is received', async () => {
    ...
    const callBack = (webSocketStore.socketManager?.socketEventEmitter.on as Mock).mock.calls[0][1]
    ...
    expect(webSocketStore.socketManager?.socketEventEmitter.on).toHaveBeenCalledWith(
      'channelMessage',
  ...
  it('requests chat room exit when leave chat button is clicked', async () => {
    ...
    expect(webSocketStore.unsubscribeFromChannel).toHaveBeenCalledWith(channelName)
  })
  ...
  it('exit chat room when channel unsubscribed successfully message is received', async () => {
    ...
    expect(wrapper.find(selectors.chatBox).exists()).toBe(false)
    // assert started listening to incomming chat room messages
    expect(webSocketStore.socketManager?.socketEventEmitter.off).toHaveBeenCalledWith(
      'channelMessage',
      (wrapper.vm as any).handleChatRoomIncomingMessages
    )
  })
  ...
  it('sends a chat message when inside a chat room', async () => {
    ...
    expect((wrapper.find(selectors.messageInput).element as HTMLInputElement).value).toBe(
      messageText
    )
    // send message
  ...
  it('receives chat messages when inside a chat room', async () => {
    ...
    expect(messageContainer.find(selectors.chatMessageText).text()).toBe(chatMessage1.text)
    expect(messageContainer.find(selectors.chatMessageUser).text()).toBe(chatMessage1.user)
    // mock event emitter triggering handleChatRoomIncomingMessages on new chat message
    ...
    expect(messageContainer.find(selectors.chatMessageText).text()).toBe(chatMessage2.text)
    expect(messageContainer.find(selectors.chatMessageUser).text()).toBe(chatMessage2.user)
  })
})
