...
describe('views > VerifyEmail.vue', () => {
  ...
  it('handles successful email verification', async () => {
    ...
    expect((wrapper.find(selectors.verificationCodeInput).element as HTMLInputElement).value).toBe(
  ...
  it('disables submit button if entered verification code is not a 6 digit number', async () => {
    ...
    for (const invalidCode in invalidCodes) {
      ...
      expect(
        (wrapper.find(selectors.verificationCodeInput).element as HTMLInputElement).value
  ...
  it('resends email when resend email button is clicked', async () => {
    ...
    expect(wrapper.find(selectors.resendEmailButton).attributes('disabled')).toBeUndefined()
    // resend email
    ...
    const countdownButtonText = (countdown: number) => `Try again in ${countdown} seconds`
    expect(wrapper.find(selectors.resendEmailButton).text()).toBe(countdownButtonText(30))
    // assert resend button is disabled
    expect(wrapper.find(selectors.resendEmailButton).attributes('disabled')).toBeDefined()
  })
