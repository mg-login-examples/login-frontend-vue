...
describe('api > modules > quotes.ts', () => {
  ...
  it('gets user quotes by userId', async () => {
    ...
    await expect(quotesApi.getUserQuotes(fakeUser.id)).resolves.toEqual(fakeUserQuotes)
  ...
  it('creates a user quote', async () => {
    ...
    await expect(quotesApi.createQuote(quoteCreate)).resolves.toEqual(fakeQuote)
  ...
  it('edits a user quote', async () => {
    ...
    expect(http.put).toHaveBeenCalledWith(`/api/quotes/${fakeQuote.id}/`, quoteEdit)
  })

  it('deletes a user quote', async () => {
    http.delete = vi.fn().mockImplementation(mockAxiosDeleteUserQuote)
    await expect(quotesApi.deleteQuote(fakeQuote.id)).resolves.toEqual(undefined)
  ...
  it('likes a user quote', async () => {
    ...
    const userId = 22
    const quoteId = 33
    await expect(quotesApi.likeQuote(quoteId, userId)).resolves.toEqual(undefined)
    expect(http.put).toHaveBeenCalledWith(`/api/quotes/${quoteId}/users/${userId}/like/`)
  })
  ...
  it('unlikes a user quote', async () => {
    ...
    const userId = 22
    const quoteId = 33
    await expect(quotesApi.unlikeQuote(quoteId, userId)).resolves.toEqual(undefined)
    expect(http.delete).toHaveBeenCalledWith(`/api/quotes/${quoteId}/users/${userId}/like/`)
  })
})
