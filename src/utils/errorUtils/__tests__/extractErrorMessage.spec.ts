...
describe('Utils > errorUtils > extractErrorMessage.ts', () => {
  ...
  it.todo('extract unknown error in non development environment', () => {
    ...
    expect(extractErrorMessage(unknownError)).toBe('Something went wrong! Please try again later.')
  })
  ...
  it('handles unknown error in development environment', () => {
    ...
    expect(extractErrorMessage(unknownError)).toBe(`Unmapped error: ${errorMessage}`)
  })
})
