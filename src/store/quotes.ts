...
export const useQuotesStore = defineStore('quotes', {
  ...
  getters: {
    userQuoteById: (state) => {
      return (quoteId: number) => state.userQuotes.find((quote) => quote.id === quoteId)
    }
  },
  actions: {
    async getQuotes() {
    ...
    async getUserQuotes() {
      ...
      if (userStore.user) {
        try {
          this.userQuotes = await backendApi.quotes.getUserQuotes(userStore.user.id)
        } catch (error) {
          const errorStore = useErrorsStore()
          errorStore.handleError(error)
          this.userQuotes = []
    ...
    async editUserQuote(quoteEdited: Quote) {
      try {
        ...
        this.userQuotes = this.userQuotes.map((q) => (q.id !== quoteEdited.id ? q : quoteEdited))
      } catch (error) {
        const errorStore = useErrorsStore()
        errorStore.handleError(error)
      }
    ...
    async deleteUserQuote(quoteId: number) {
      try {
        ...
        this.userQuotes = this.userQuotes.filter((quote) => quote.id !== quoteId)
      } catch (error) {
        const errorStore = useErrorsStore()
        errorStore.handleError(error)
      }
    ...
    async unlikeQuote(quoteId: number) {
      ...
      if (userStore.user) {
        try {
          ...
          this.quotes = this.quotes.map((quote) => {
            if (quote.id === quoteId) {
              quote.liked_by_users = quote.liked_by_users.filter((user) => user.id !== user.id)
            }
            return quote
          })
        } catch (error) {
