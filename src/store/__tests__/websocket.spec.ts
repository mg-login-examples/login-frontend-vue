...
describe('store > websocket.ts', () => {
  ...
  it('inits the websocket store with no socket manager', () => {
    ...
    // assert init values
    expect(socketStore.socketUrl).toBe(import.meta.env.VITE_APP_BACKEND_WEBSOCKET_URL)
  ...
  it('resets socket channels subscribers when disconnectSocketIfConnected is called', () => {
    ...
    expect(mockSocketManager.socketEventEmitter.off).toBeCalledWith('socketConnected')
  ...
  it('subscribes to a channel and stores callback to resubscribe on websocket unexpected close', () => {
    ...
    expect(socketStore.socketEventEmitter?.on).toBeCalledWith('socketConnected', expect.anything())
