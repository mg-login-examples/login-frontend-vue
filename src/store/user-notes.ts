...
export const useUserNotesStore = defineStore('userNotes', {
  ...
  getters: {
    userNoteById: (state) => {
      return (userNoteId: string) => state.userNotes.find((userNote) => userNote.id === userNoteId)
    }
  },
  actions: {
    async getUserNotes() {
      ...
      if (userStore.user) {
        try {
          this.userNotes = await backendApi.userNotes.getUserNotes(userStore.user.id)
        } catch (error) {
    ...
    async createUserNote(userNoteCreate: UserNoteCreate) {
      ...
      if (userStore.user) {
        try {
          const newUserNote = await backendApi.userNotes.createUserNote(userNoteCreate)
    ...
    async deleteUserNote(userNoteId: string) {
      try {
        ...
        this.userNotes = this.userNotes.filter((userNote) => userNote.id !== userNoteId)
      } catch (error) {
        const errorStore = useErrorsStore()
        errorStore.handleError(error)
      }
