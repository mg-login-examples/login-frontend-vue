...
describe('components > generic > modal > AppModal.vue', () => {
  it('renders modal when showModal is true', async () => {
    ...
    expect(appModalContentWrapper.find(selectors.appModalInnerContent).exists()).toBe(true)
  })
  ...
  it('closes modal only when clicking outside the central focus element', async () => {
    ...
    expect(appModalContentWrapper.element.parentElement.getAttribute('data-test')).toBe(
      'modal--focus-element-wrapper'
    )
    ...
    expect(appModalContentWrapper.element.parentElement.parentElement.tagName).toBe(
      'TRANSITION-STUB'
    )
