...
describe('components > generic > switches > SimpleSwitch', () => {
  it('toggles v-model bound boolean value when switch button is clicked', async () => {
    // mount switch
    const wrapper = mount(SimpleSwitch as Component, {
      global: {
  ...
  it('sets default switch container background if none is provided', async () => {
    ...
    expect(wrapper.find(selectors.switchContainer).classes()).to.include.members([
      'bg-gray-300',
      'dark:bg-gray-700'
    ])
    // turn on switch
    ...
    expect(wrapper.find(selectors.switchContainer).classes()).to.include('bg-green-400')
  })
  ...
  it('sets switch container background class if provided as prop', async () => {
    ...
    expect(wrapper.find(selectors.switchContainer).classes()).not.to.include.members(
      bgOnClasses.split(' ')
    )
    ...
    // assert default classes are not set
    expect(wrapper.find(selectors.switchContainer).classes()).not.to.include('bg-green-400')
  })
  ...
  it('sets switch button background class if provided as prop', async () => {
    ...
    // assert when switch off, custom classes for switch on are not set
    expect(wrapper.find(selectors.switchButton).classes()).not.to.include.members(
      bgOnClasses.split(' ')
    )
    // assert default classes are not set
    expect(wrapper.find(selectors.switchButton).classes()).not.to.include.members([
  ...
  it('sets switch-on icon if switchIcon or switchOnIcon is provided', async () => {
    ...
    expect(wrapper.find(selectors.switchOnIcon).attributes('icon')).toBe(switchIcon)

    // set switchOnIcon
    await wrapper.setProps({ switchOnIcon: switchOnIcon })
    // assert switch-on icon has expected value
    expect(wrapper.find(selectors.switchOnIcon).attributes('icon')).toBe(switchOnIcon)
  })
  ...
  it('sets switch-off icon if switchIcon or switchOffIcon is provided', async () => {
    ...
    expect(wrapper.find(selectors.switchOffIcon).attributes('icon')).toBe(switchIcon)

    // set switchOnIcon
    await wrapper.setProps({ switchOffIcon: switchOffIcon })
    // assert switch-on icon has expected value
    expect(wrapper.find(selectors.switchOffIcon).attributes('icon')).toBe(switchOffIcon)
  })
