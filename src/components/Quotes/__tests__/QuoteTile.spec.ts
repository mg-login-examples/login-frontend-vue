...
describe('components > QuoteTile.vue', () => {
  ...
  it('renders author username if only when prop myQuote value is false', () => {
    ...
    expect(wrapper.find(selectors.quoteAuthorUsername).text()).toBe(`- ${expectedQuoteUsername}`)
  })
  ...
  it("selects tile background based on prop myQuote's value", async () => {
    ...
    expect(wrapper.find(selectors.quoteTile).classes(allQuoteBackgroundClass)).toBe(true)
    expect(wrapper.find(selectors.quoteTile).classes(userQuoteBackgroundClass)).toBe(false)

    wrapper.setProps({ myQuote: true })
    await Vue.nextTick()
    expect(wrapper.find(selectors.quoteTile).classes(userQuoteBackgroundClass)).toBe(true)
    expect(wrapper.find(selectors.quoteTile).classes(allQuoteBackgroundClass)).toBe(false)
  })
