...
export class LoginWebAppUserFactory {
  ...
  async getRandomUser() {
    const randomEmail = minifaker.email().toLowerCase();
    const randomPasssword = minifaker.password({ minLength: 8 });
    const userName = randomEmail;
    await DataSetterHelpers.createUserIfNoUserExists(
      randomEmail,
      randomPasssword
    );
    const browserContext = await this.browser.newContext();
    const page = await browserContext.newPage();
    const newUser = new LoginWebAppUser(
      userName,
      randomEmail,
      randomPasssword,
      page
