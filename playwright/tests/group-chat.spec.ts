...
test('Send and receive messages in a group chat', async ({ loginWebAppUserFactory }) => {
  const user1 = await loginWebAppUserFactory.getRandomUser()
  await user1.login()
  await user1.onGroupChatView.open()
  await user1.withGroupChatTask.enterChatRoom('secret meeting')
  ...
  await user2.onGroupChatView.messageWithUser(chatMessage1, user1.email).expectToBeVisible()

  const chatMessage2 = `Yo ${user1.name}! Nice to see you`
  await user2.withGroupChatTask.sendMessage(chatMessage2, user2.email)
  await user1.onGroupChatView.messageWithUser(chatMessage2, user2.email).expectToBeVisible()
