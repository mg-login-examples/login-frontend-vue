...
export class LoginViewScreen {
  ...
  static rememberMeCheckboxSelector =
  ...
  static loginShowPasswordButtonSelector =
  ...
  static loginShowPasswordButtonIconSelector =
  ...
  async expectToNotBeOpen() {
    const loginUrl = "/login";
    const stepTitle = userStepTitle(
      `Expect login page to not be open`,
      this.userName
    );
    await test.step(stepTitle, async () => {
      await expect(async () => {
