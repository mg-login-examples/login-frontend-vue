...
export class GroupChatTasks {
  ...
  async enterChatRoom(roomName: string) {
    ...
    await test.step(stepTitle, async () => {
      ...
      await this.loginWebApp.onGroupChatView.roomNameInput.expectToHaveValue(roomName)
      await this.loginWebApp.onGroupChatView.enterRoomNameButton.click()
      await this.loginWebApp.onGroupChatView.chatRoomTitle.expectToHaveText(roomName)
    })
  }
  ...
  async sendMessage(chatMessage: string, senderName: string) {
    ...
    await test.step(stepTitle, async () => {
      ...
      await this.loginWebApp.onGroupChatView.chatMessageInput.expectToHaveValue(chatMessage)
